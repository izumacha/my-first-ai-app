...
- `src/lib/rate-limit.ts` — 送信元ごとのスライディングウィンドウ・レート制限（上限値・追跡表・共有バケット）。Route Handler は HTTP メソッド以外を export できずテストから触れないため、判定ロジックと状態をここへ切り出し、上限値と現在時刻を注入できるようにしている（満杯時の経路を単体テストで再現するため）。**共有バケット（満杯時の受け皿）は追跡表の外に持つ** — 表の中にキーとして混ぜると件数が上限を 1 つ超え、上限が小さいときは共有バケットだけで表が埋まって実クライアントが自分のバケットを持てなくなる。**429 の `Retry-After` は制限器インスタンスの `windowMs` から導く**（既定値を読むと上限を差し替えたときにヘッダだけ古い値のまま残る）。**追跡表の掃除は 1 ウィンドウにつき 1 回まで**に絞る — 送信元キーは偽装できるので、絞らないと使い捨てキーで表を満杯にするだけで以後の全リクエストに 1 万件の走査を負担させられる（代わりに、期限切れから回収までが 1 ウィンドウ未満だけ遅れる。遅れている間の新規送信元は共有バケットで数えるので素通りにはならない）。**上限値は生成時に「正の整数」であることを検証して落とす** — `windowMs` が 0 だと記録が常に空になりレート制限が丸ごと無効になる（このアプリの DoS・課金対策はこれしか無いので fail-closed にする）。
