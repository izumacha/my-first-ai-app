...
- **ストリーミング開始後（200 とヘッダを返し終えたあと）の失敗は上のステータス契約では扱えない。** `createSseStream` の中で起きるため POST の `catch`（`mapErrorToResponse`）へは戻らず、`controller.error()` は受信側を reject させるだけでサーバ側には何も残らない。中断以外の失敗は**必ず `console.error` でサーバログに残す**。
  ...
  - **画面側も「途中で切れた回答」に印を付ける**（`chat-limits.ts` の `TRUNCATED_ANSWER_SUFFIX`。`[DONE]` を受け取れなかった読み取りが該当）。印が無いと見た目は完全な回答と変わらず、次の質問でその欠けた回答が文脈として送り返される。**空白だけの回答は履歴に残さない** — 残すとサーバーの検証（本文が空）で以降の送信がすべて 400 になり、往復が成立しないので 50 件の窓からも抜けず復帰できなくなる（`trimHistoryForRequest` も同じ理由で空白だけの発言を落とす）。ただし**黙って終わらせない**: 1 文字も受け取れなかったときは理由を表示する（画面に何も出ないと、ユーザーは失敗に気づかず再送して上流の呼び出しを重ねる）。**解析できずに読み飛ばした差分があった回答も「未完了」として扱う** — `[DONE]` は普通に届くので、読み飛ばした事実を覚えていないと欠けのある回答が完全な回答として確定する。
