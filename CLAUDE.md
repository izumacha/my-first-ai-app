...
**スタック:** Next.js 15 (App Router), TypeScript, Anthropic Claude API (claude-sonnet-4-6), Vercel AI SDK, Tailwind CSS v4, Vitest。
...
- API ルートに簡易レート制限（IP ベース、1 分あたり 20 リクエスト目安）。エラーは HTTP ステータスを使い分ける: 401（キー未設定/無効）/ 429（レート制限超過）/ 500（その他）。フロントはユーザーフレンドリーな日本語メッセージを表示する。
