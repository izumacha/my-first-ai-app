...
- `src/lib/rate-limit.ts` — 送信元ごとのスライディングウィンドウ・レート制限（上限値・追跡表・共有バケット）。Route Handler は HTTP メソッド以外を export できずテストから触れないため、判定ロジックと状態をここへ切り出し、上限値と現在時刻を注入できるようにしている（満杯時の経路を単体テストで再現するため）。**共有バケット（満杯時の受け皿）は追跡表の外に持つ** — 表の中にキーとして混ぜると件数が上限を 1 つ超え、上限が小さいときは共有バケットだけで表が埋まって実クライアントが自分のバケットを持てなくなる。**上流の 429 を中継するときは上流の `Retry-After` に従う**（自前の待機時間を返すと、クライアントは成功しえない再試行を繰り返して自前の枠まで食い潰す）。自前のレート制限で弾いたときの **`Retry-After` は制限器インスタンスの `retryAfterSeconds` から導く**（既定値を読むと上限を差し替えたときにヘッダだけ古い値のまま残る。秒への切り上げも制限器側で行う — RFC 9110 の delay-seconds は整数で、小数を送るとヘッダが無いのと同じ扱いになる）。**追跡表の掃除は一定間隔（ウィンドウの 1/60。既定なら 1 秒）に絞る** — 送信元キーは偽装できるので、絞らないと使い捨てキーで表を満杯にするだけで以後の全リクエストに 1 万件の走査を負担させられる。**「1 ウィンドウに 1 回」で絞ってはいけない**: そのウィンドウの早い時点（まだ何も期限切れでない時点）で掃除を使い切ると、その後に期限切れになっても次のウィンドウまで回収されず、「回収できる枠しかないのに満杯」の状態で正規の新規クライアントが共有バケットへ押し込まれて 429 を受ける。間隔で絞れば遅れは常に 1 間隔以内。**間隔を長くしすぎない** — 表が満杯の間の新規クライアントは共有バケット（全体で 1 枠）へ回されるので、回収の遅れはそのまま新規クライアントの 429 に化ける。掃除の頻度はリクエスト数から切り離されている必要があるが判定結果には現れないため、`sweepCount` を公開して観測できるようにしている。**上限値は生成時に「正の整数」であることを検証して落とす** — `windowMs` が 0 だと記録が常に空になりレート制限が丸ごと無効になる（このアプリの DoS・課金対策はこれしか無いので fail-closed にする）。
