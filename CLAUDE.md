...
- **ストリーミング開始後（200 とヘッダを返し終えたあと）の失敗は上のステータス契約では扱えない。** `createSseStream` の中で起きるため POST の `catch`（`mapErrorToResponse`）へは戻らず、`controller.error()` は受信側を reject させるだけでサーバ側には何も残らない。中断以外の失敗は**必ず `console.error` でサーバログに残す**（日常的に起きる中断はログを埋めるだけなので記録しない）。中断時は「受信側が `cancel()` で切断したか」を区別し、切断していなければ `controller.close()` で終端を通知する — 何もせずに抜けると、`request.signal` 由来の中断でストリームが完結も失敗もしないまま宙吊りになり、読み手の `read()` が永久に解決しない（`cancel()` 済みのストリームを `close()` すると TypeError になるため、区別せず一律に閉じることはできない）。
