...
- **ストリーミング開始後（200 とヘッダを返し終えたあと）の失敗は上のステータス契約では扱えない。** `createSseStream` の中で起きるため POST の `catch`（`mapErrorToResponse`）へは戻らず、`controller.error()` は受信側を reject させるだけでサーバ側には何も残らない。中断以外の失敗は**必ず `console.error` でサーバログに残す**。
  - **切断の判定に「反復中に中断エラーが飛ぶこと」を期待しない。** SDK のストリームは反復中の中断を throw せず正常終了する（`@anthropic-ai/sdk/core/streaming.js` の「abort されたら return する」）。そのため受信側が切断しても `for await` は例外を出さずに抜け、切断は**直後の終端フレーム `enqueue` が `Invalid state: Controller is already closed` の TypeError になる形で初めて表面化する**。これは正常な切断の結果であって障害ではないので、`cancel()` が呼ばれたかを記録して `catch` の先頭で早期 return する。ここを誤ると、通常のタブ閉じのたびに障害ログが積まれて本物の上流障害が埋もれる（**この経路が最も頻度が高い**）。
  - 受信側が切断していない中断（SDK の分類が変わった場合の保険）では、まだ読み手がいるので `controller.close()` で終端を伝える。何もせず抜けるとストリームが完結も失敗もしないまま宙吊りになり、読み手の `read()` が永久に解決しない。切断済みの経路は上の早期 return で抜けているため、この `close()` は必ず有効（`cancel()` 済みのストリームを `close()` すると TypeError になる）。
