...
- **ストリーミング開始後（200 とヘッダを返し終えたあと）の失敗は上のステータス契約では扱えない。** `createSseStream` の中で起きるため POST の `catch`（`mapErrorToResponse`）へは戻らず、`controller.error()` は受信側を reject させるだけでサーバ側には何も残らない。中断以外の失敗は**必ず `console.error` でサーバログに残す**。
  ...
  - **`[DONE]` は「回答が完結した」という意味**で、途中で終わったときは送らない。判定は `message_delta` の `stop_reason` で行うが、**打ち切りの理由を列挙して弾くのではなく、最後まで話し終えた理由（`end_turn` / `stop_sequence`）だけを完了とみなす**。`StopReason` には `max_tokens` のほかに `model_context_window_exceeded` や `refusal` もあり、今後も増えうる。列挙側で持つと、増えた理由が黙って「完全な回答」に化ける。途中終了のときは番兵を送らずに `close()` するだけにする（エラーにはしない — 実際に届いている回答に通信障害の警告を出すのは誤り）。
