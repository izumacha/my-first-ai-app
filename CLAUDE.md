...
- **ストリーミング開始後（200 とヘッダを返し終えたあと）の失敗は上のステータス契約では扱えない。** `createSseStream` の中で起きるため POST の `catch`（`mapErrorToResponse`）へは戻らず、`controller.error()` は受信側を reject させるだけでサーバ側には何も残らない。中断以外の失敗は**必ず `console.error` でサーバログに残す**。
  ...
  - **読み切ったあとに `requestSignal.aborted` を確認する。** 中断されていたら `[DONE]` を送らずに `controller.error()` で終える。中断エラーが例外として飛んできた場合（SDK の分類が変わったときの保険）も、受信側が残っていれば同じく `controller.error()` — 黙って `close()` すると読み手は `done` で抜けて完全な回答として確定させてしまい、正常完了と区別が付かない。**この経路ではログを出さない**: タブ閉じでも `request.signal` は中断され、`cancel()` が届く順序に保証が無いため、「離脱」と「プラットフォームの打ち切り」を区別できない（出すと通常の離脱のたびに障害ログが積まれる）。上流そのものの失敗は別経路で必ず記録される。
