...
- API ルートに簡易レート制限（IP ベース、1 分あたり 20 リクエスト目安）。エラーは HTTP ステータスを使い分ける: 400（JSON 破損・入力検証エラー・上流 Claude API の 400）/ 401（キー未設定/無効）/ 413（本文サイズ上限超過。Content-Length の申告値と実際の読み取りバイト数の両方で強制）/ 429（レート制限超過、`Retry-After` ヘッダ付き）/ 500（その他）。フロントはユーザーフレンドリーな日本語メッセージを表示する。
