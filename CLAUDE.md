...
- **「実際に動かす Node の major」は 5 か所で揃える。** 宣言は `.nvmrc` / `.github/workflows/ci.yml` の `NODE_VERSION` / `Dockerfile` の `FROM node:<major>`（全ステージ）/ `package.json` の `engines.node` / README の必要環境に分かれている。**Node を次の major へ上げるときは 5 か所を同じ major へ揃える PR を 1 本で出し、同じ PR で `@types/node` も新しい major へ上げる。** ずれても **lint も型チェックもテストも通る** — npm は `engines` を既定で強制しないので、依存がサポートしない Node の上でも `npm ci` は成功し、テストも「たまたま動いている間は」緑になる。**CI の緑が「その Node で動く」ことの証明にならない fail-open** で、壊れるのは出荷先の実行時。実際このリポジトリは Dockerfile が `node:26-alpine` で出荷する一方、CI の matrix が `[20, 22]` のまま（＝出荷する Node を一度も検証していない。しかも Node 20 は `@testing-library/jest-dom` の `engines: >=22` を満たさない）という状態だった。`@types/node` の major 更新は `.github/dependabot.yml` の `ignore` で止めてある（型だけが先に進むと、実行時に無い API を書いても `tsc` が緑のまま通るため。**この `ignore` を消さない**。ランタイムを上げる PR で一緒に上げる）。整合性は `tests/node-runtime-alignment.test.ts` が機械的に見張る: (a) ピン留め 3 か所の食い違い・読み取り不能（前提が崩れるので fail-closed で落とす）、(b) `engines.node` / README / `@types/node`（宣言と解決済み版の両方）のずれ、(c) **直接依存が `engines.node` で要求する Node をピンが満たしているか**（上の「サポート外の Node で緑」を直接落とす検査）、(d) `@types/node` の保留の消失・効きすぎ・置き場所間違い・ワイルドカード化。設定ファイルの読み方（`directories` の複数形・glob、`dependency-name` のワイルドカード、複数エントリがすべて適用されること）は `tests/lib/dependabot-config.ts` に集約し、ESLint 保留のガードと共有する（写しを持つと片方だけ直されて、もう片方の検出網が静かに緩む）。
