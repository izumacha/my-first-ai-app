...
- `src/lib/sse.ts` — SSE のフレーム書式（`data: ` プレフィックス・`[DONE]` 番兵）と、その整形／解析ヘルパー。**送信側（`route.ts`）と読み取り側（`page.tsx`）の唯一の参照元**で、書式の直書きはどちらにも置かない。片側だけ変えると型チェックを通ったまま解析が壊れるため、`tests/sse.test.ts` の契約テストが両者の一致を機械的に守る。**行末の CR は解析側で落とす** — SSE の行区切りは CRLF・CR も認められており、途中のプロキシが CRLF で流すと本文の末尾に `\r` が残る。JSON としては CR は空白なので本文は届いてしまい、番兵との文字列比較だけが一致しない＝**完全な回答が毎回「中断された」と誤判定される**。
...
- **ストリーミング開始後（200 とヘッダを返し終えたあと）の失敗は上のステータス契約では扱えない。** `createSseStream` の中で起きるため POST の `catch`（`mapErrorToResponse`）へは戻らず、`controller.error()` は受信側を reject させるだけでサーバ側には何も残らない。中断以外の失敗は**必ず `console.error` でサーバログに残す**。
  ...
  - **`[DONE]` は「回答が完結した」という意味**で、途中で終わったときは送らない。**`max_tokens` による打ち切り**（`message_delta` の `stop_reason`）はこのアプリでいちばん起きやすい打ち切り理由で、通信としては正常に終わるため見落としやすい。この場合は番兵を送らずに `close()` するだけにする（エラーにはしない — 実際に届いている回答に通信障害の警告を出すのは誤り）。
