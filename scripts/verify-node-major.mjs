...
function fail(message) {
  ...
  // 終了コード 1** になり、この関数がまさに避けようとしている状態そのものになる。
  // 2 つの出口で同じ手当てをするため、退避はループの外に 1 つだけ置く
  if (written < payload.length) {
    // **書き込みが途中で終わっても文言を捨てない。** stderr がノンブロッキングな
    // パイプ (CI のログ収集がこの形) でバッファが満杯だと、writeSync は EAGAIN を
    // 投げるか 0 バイトを返す。そのまま抜けると、丁寧に書いた理由が欠けたまま
    // 終了コード 1 になる — この関数がまさに避けようとしている状態そのもの。
