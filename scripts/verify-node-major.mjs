...
//       (実測でいずれも素通りしていたため塞いだ)。
//   **この 2 つは、テストスイート自身が `process.versions.node` をピンと突き合わせる
//   ことで塞いである** (`tests/node-runtime-alignment.test.ts` の
//   「このスイートを走らせている Node が、ピン留めした major と同じ」)。
//   lint / typecheck / test が実際に走るのはまさに「この検証より後ろ」なので、
//   その Node 自身に名乗らせれば綴りに依存せず分かる。
//   **このスクリプトは、その前段 (`npm ci` より前) を覆う役割で残す** ——
//   依存の解決そのものが別の Node で行われる形は、スイートが動き出す前に起きるため。
