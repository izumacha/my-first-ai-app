...
describe("追跡表が満杯のときの挙動", () => {
  ...
  it("共有バケットで数えた分は、表に空きができても引き継ぐ", () => {
    ...
    // 弾かれたあとも枠は戻らない。引き取った記録を「弾いたから」と書き戻さずに
    // 捨てると、共有バケットからは取り除いてあるのでどこにも残らず、次の
    // リクエストが空から数え直しになって同じ穴が開く（弾かれた側の経路で再発する）
    expect(limiter.isRateLimited("k", 1006)).toBe(true);
    expect(limiter.isRateLimited("k", 1007)).toBe(true);
  });
